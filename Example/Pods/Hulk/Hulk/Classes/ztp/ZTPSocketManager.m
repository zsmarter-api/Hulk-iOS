...
- (void)connect
{
    NSLog(@"Hulk socket start connect to %@:%@", self.server, @(self.port));
...
- (void)handshake
{
    NSLog(@"Hulk socket do handshake");
...
- (void)sendHeartBeat
{
    NSLog(@"Hulk socket send heartbeat");
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)t
{
    ...
    @try {
        ...
        NSLog(@"Hulk socket did read data type %@ subtype %@ ack %@ seq %@", @(message.type), @(message.subType), @(message.ack), @(message.seq));
        if (message.ack == 1) {
            [self sendDataConfigBlock:^(ZTPData * _Nonnull data) {
        ...
        NSLog(@"Hulk socket exception: %@", exception.description);
    }
}

- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err
{
    NSLog(@"Hulk socket did disconnect");
